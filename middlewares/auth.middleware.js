  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (updateInfo === null || updateInfo.modifiedCount === 0) {
        throw Error('Something went wrong while logging you in, please try again');
      }
...
const decode_jwt = function (req, res, type) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const accessToken = jwt.verify(req.cookies.umis_siwesA, process.env.ACCESS_TOKEN_SECRET);
      ...
      try {
        ...
        const refreshToken = jwt.verify(req.cookies.umis_siwesR, process.env.REFRESH_TOKEN_SECRET);
  
        await verify_token_status(refreshToken, type);
  
        // refresh token is valid
        const data = await assign_new_tokens({ _id: refreshToken.id }, res, type);
