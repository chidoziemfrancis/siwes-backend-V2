...
const get_a_supervisor = async function(req, res) {
  try {
    ...
    const supervisor = await SUPERVISORS.findOne({ _id: id }, { password: 0, validation_secret: 0, createdAt: 0, updatedAt: 0 });
...
const get_forms = async function(req, res) {
  try {
    ...
    if (forms.length === 0) {
      return res.status(404).json({ message: "There are currently no forms available" });
    }
...
const change_password = async function(req, res) {
  ...
  try {
    ...
    if (supervisor === null) {
      return res.status(401).json({ message: "Something unusual happened to your authentication status while trying to chaneg your password, so we couldn't process your request" })
    }

    const passwordIsValid = await bcrypt.compare(oldPassword, supervisor.password);
    ...
    const { modifiedCount } = await SUPERVISORS.updateOne({ _id }, { password: hashedPassword });
